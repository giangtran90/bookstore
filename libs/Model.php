...
class Model
{
    ...
    public function fetchAll($query)
    {
        ...
        if (!empty($query)) {
            ...
            if (mysqli_num_rows($resultQuery) > 0) {
                ...
                mysqli_free_result($resultQuery); // giải phóng dung lượng bộ nhớ
            }
        }      
        return $result;
    }
    ...
    // TOTAL ITEMS
    public function totalItems($query)
    {
        if (!empty($query)){
            $resultQuery = $this->query($query);
            if (mysqli_num_rows($resultQuery) > 0) {
    ...
    public function injection($id){
        $this->id = mysqli_real_escape_string($this->connect,$id);
        return $this->id;
    }
