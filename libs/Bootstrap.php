...
class Bootstrap
{
    ...
    public function init()
    {
        ...
        if (file_exists($filePath)) {
            ...
            URL::redirect('default', 'index', 'notice', ['type' => 'not-url']);
        }
    }
    // CALL METHODE
            ...
            if ($module == 'admin') {
                if ($logged == true) {
                    if ($userInfo['group_acp'] == 1) {
                        ...
                        // } else {
                        //     URL::redirect('default', 'index', 'notice', ['type' => 'not-permission']);
                        // }                  
                    } else {
                        URL::redirect('default', 'index', 'notice', ['type' => 'not-permission']);
                    }
    ...
    public function setParam()
    {
        ...
        $this->_params['module']        = ($this->_params['module']) ?? DEFAULT_MODULE;
        $this->_params['controller']    = ($this->_params['controller']) ?? DEFAULT_CONTROLLER;
        $this->_params['action']        = ($this->_params['action']) ?? DEFAULT_ACTION;
    }
    // CALL LOGIN ACTION
